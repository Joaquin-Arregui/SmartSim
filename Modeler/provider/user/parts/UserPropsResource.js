...
export default function(element) {
  ...
  return [
    ...
    {
      id: 'minimumTimeByUser',
      element,
      component: minimumTimeByUserFunction,
      isEdited: el => Array.isArray(el.businessObject?.minimumTimeByUser) && el.businessObject.minimumTimeByUser.length > 0
    },
    {
      id: 'maximumTimeByUser',
      element,
      component: maximumTimeByUserFunction,
      isEdited: el => Array.isArray(el.businessObject?.maximumTimeByUser) && el.businessObject.maximumTimeByUser.length > 0
    },
    {
      id: 'loopParameter',
      element,
...
function minimumTimeByUserFunction(props) {
  const { element } = props;
  const modeling = useService('modeling');
  const translate = useService('translate');
  const moddle = useService('moddle');

  const isCollab = !!element?.businessObject?.participants;
  const getBOsToWrite = () => {
    if (!isCollab) return [{ target: element, isProcessRef: false }];
    const parts = element.businessObject.participants || [];
    return parts
      .filter(p => p.processRef)
      .map(p => ({ target: p.processRef, isProcessRef: true }));
  };

  const getPairs = () => {
    if (isCollab) {
      const first = element.businessObject.participants?.[0]?.processRef;
      const arr = first?.minimumTimeByUser;
      return Array.isArray(arr) ? arr : [];
    }
    const arr = element?.businessObject?.minimumTimeByUser;
    return Array.isArray(arr) ? arr : [];
  };

  const setPairs = (updated) => {
    if (isCollab) {
      getBOsToWrite().forEach(({ target }) => {
        modeling.updateModdleProperties(element, target, { minimumTimeByUser: updated });
      });
    } else {
      modeling.updateProperties(element, { minimumTimeByUser: updated });
    }
  };

  const addPair = () => {
    const current = getPairs();
    const kv = moddle.create('model:KeyValuePair', { key: `user${current.length + 1}`, value: 0 });
    setPairs([...current, kv]);
  };

  const setKey = (index, newKey) => {
    const current = getPairs();
    const item = current[index] || {};
    const kv = moddle.create('model:KeyValuePair', { key: newKey, value: Number.isInteger(item.value) ? item.value : 0 });
    const updated = [...current];
    updated[index] = kv;
    setPairs(updated);
  };

  const setValue = (index, newVal) => {
    // fuerza Integer
    const parsed = Number.parseInt(newVal, 10);
    if (!Number.isInteger(parsed) || parsed < 0) return; // solo enteros >= 0

    const current = getPairs();
    const item = current[index] || {};
    const kv = moddle.create('model:KeyValuePair', { key: item.key ?? '', value: parsed });
    const updated = [...current];
    updated[index] = kv;
    setPairs(updated);
  };
  ...
  const render = () => getPairs().map((pair, i) => html`
    <div class="user-pool-item">
      <input
        type="text"
        value=${pair.key || ''}
        onInput=${(e) => setKey(i, e.target.value)}
      ...
      <input
        ...
        step="1"
        min="0"
        value=${Number.isInteger(pair.value) ? pair.value : 0}
        onInput=${(e) => setValue(i, e.target.value)}
        placeholder="${translate('Min time (int)')}"
      />
      <button
  ...
  return html`
    <div class="user-pool-container">
      ${render()}
      <button class="add-role-button" onClick=${addPair}>
        ${translate('Add user min time')}
      </button>
    </div>
  `;
}

function maximumTimeByUserFunction(props) {
  const { element } = props;
  const modeling = useService('modeling');
  const translate = useService('translate');
  const moddle = useService('moddle');
  ...
  const getPairs = () => {
    if (isCollab) {
      const first = element.businessObject.participants?.[0]?.processRef;
      const arr = first?.maximumTimeByUser;
      return Array.isArray(arr) ? arr : [];
    }
    const arr = element?.businessObject?.maximumTimeByUser;
    return Array.isArray(arr) ? arr : [];
  };
  ...
  const setPairs = (updated) => {
    if (isCollab) {
      getBOsToWrite().forEach(({ target }) => {
        modeling.updateModdleProperties(element, target, { maximumTimeByUser: updated });
      });
    } else {
      modeling.updateProperties(element, { maximumTimeByUser: updated });
    }
  };
  ...
  const render = () => getPairs().map((pair, i) => html`
    <div class="user-pool-item">
      ...
      <input
        ...
        value=${Number.isInteger(pair.value) ? pair.value : 0}
        onInput=${(e) => setValue(i, e.target.value)}
        placeholder="${translate('Max time (int)')}"
      />
      <button
        class="remove-role-button"
        onClick=${() => removePair(i)}
