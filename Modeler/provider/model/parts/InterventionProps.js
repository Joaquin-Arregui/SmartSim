...
function addUserFunction(props) {
  return userEditor(props, 'addUser', 'Add new user');
}

function deleteUserFunction(props) {
  return userEditor(props, 'deleteUser', 'Delete user');
}

function userEditor(props, attribute, buttonLabel) {
  const { element } = props;
  ...
  const addRole = () => {
    ...
    const newEntry = moddle.create('model:KeyValuePair', {
      key: '',
      value: ''
    });
