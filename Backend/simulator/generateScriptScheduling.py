...
def generateScriptScheduling(elements, process, starts, messageStarts):
...
dic = {{}}
for key, value in simulationResults.items():
    eventDic = {{}}
    standBys = {{}}
    for item in value:
        if "<event>" in item:
...
''')
            for line in lines:
                if 'bpmn:type' in line:
                    BPMNType = line.split('value="')[1].split('"')[0]
                if 'bpmn:id' in line:
                    id = line.split('value="')[1].split('"')[0]
                    if id not in eventDic:
                        eventDic[id] = {{}}
                elif 'bpmn:userTask' in line:
                    properties['bpmn:userTask'] = line.split('value="')[1].split('"')[0]
                elif 'bpmn:execution' in line:
                    properties['bpmn:execution'] = line.split('value="')[1].split('"')[0]
                elif 'bpmn:time' in line:
                    properties['bpmn:time'] = line.split('value="')[1].split('"')[0]
                elif 'bpmn:subTask' in line:
                    subTask = line.split('value="')[1].split('"')[0]
                    properties['bpmn:subTask'] = subTask
                elif 'time:timestamp' in line:
                    if "Scheduler" == BPMNType:
                        eventDic['timeReady'] = line.split('value="')[1].split('"')[0]
                    if "StandBy" in BPMNType:
                        if BPMNType not in properties:
                            properties[BPMNType] = {{}}
                        properties[BPMNType]['time:timestamp'] = line.split('value="')[1].split('"')[0]
                    else:
                        properties['time:timestamp'] = line.split('value="')[1].split('"')[0]
                elif 'time:endTime' in line:
                    if BPMNType not in properties:
                        properties[BPMNType] = {{}}
                    properties[BPMNType]['time:endTime'] = line.split('value="')[1].split('"')[0]
            if 'bpmn:execution' in properties:
                if "StandBy" in BPMNType:
                    if id not in standBys:
                        standBys[id] = {{}}
                    if BPMNType not in standBys[id]:
                        standBys[id][properties['bpmn:execution']] = {{}}
                    standBys[id][properties['bpmn:execution']][BPMNType] = properties
                else:
                    eventDic[id][properties['bpmn:execution']] = properties
            else:
                if "StandBy" in BPMNType:
                    if id not in standBys:
                        standBys[id] = {{}}
                    standBys[id][BPMNType] = properties
                else:
                    eventDic[id] = properties
    for id, properties in standBys.items():
        for execution, standByProperties in properties.items():
            if execution in eventDic[id]:
                for k in ["StandByMessage", "StandBy"]:
                    if k in standByProperties:
                        eventDic[id][execution].update(standByProperties[k])
            else:
                eventDic[id].update(standByProperties)
    dic[key] = eventDic
with open(f'schedulingData.json', 'w') as f:
...
def generateScriptScheduled(elements, process, starts, messageStarts, schedulingResult, schedulingData):
    elementProcess = elements[process]
    if elementProcess.bpmn_type == 'bpmn:Process':
        script = f"""
import simpy
...
"""
        ...
        for start in starts:
            startEvent = elements[start]
            startElements[start] = [startEvent.bpmn_type, startEvent.name, startEvent.id_bpmn, startEvent.subTask]
            script = generateFunctionScheduler(elements, startEvent.subTask, script, schedulingResult, schedulingData)
        for start in messageStarts:
            startEvent = elements[start]
            messageStartElements[start] = [startEvent.bpmn_type, startEvent.name, startEvent.id_bpmn, startEvent.messageOrigin, startEvent.subTask]
            script = generateFunctionScheduler(elements, startEvent.subTask, script, schedulingResult, schedulingData)
