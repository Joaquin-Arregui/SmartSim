        ...
        if "<event>" in item:
            ...
            for line in lines:
                if 'bpmn:id' in line:
                    ...
                    eventDic[id]['bpmn:userTask'] = line.split('value="')[1].split('"')[0]
                elif 'bpmn:execution' in line:
                    eventDic[id]['bpmn:execution'] = line.split('value="')[1].split('"')[0]
                elif 'bpmn:time' in line:
                    eventDic[id]['bpmn:time'] = line.split('value="')[1].split('"')[0]
                elif 'bpmn:subTask' in line:
                    subTask = line.split('value="')[1].split('"')[0]
                    eventDic[id]['bpmn:subTask'] = subTask
                    if "Scheduler" in subTask:
                        eventDic['preScheduler'] = id
                elif 'time:timestamp' in line:
                    eventDic[id]['time:timestamp'] = line.split('value="')[1].split('"')[0]
