def intermediateThrowEvent(elements, element, script, schedulingResult, schedulingData):
...
def {element.id_bpmn}(env, name):
    ...
    timestamp = schedulingData[name][TaskName]["time:timestamp"]
...
def messageIntermediateCatchEvent(elements, element, script, schedulingResult, schedulingData):
    from simulator.simulationFunctionsScheduler.generateFunction import generateFunctionScheduler
    if any(element.id_bpmn in schedulingDataByInstance for schedulingDataByInstance in schedulingData.values()):
        functionStr = f"""
def {element.id_bpmn}(env, name):
    ...
    schedulingData = {schedulingData}
    timestamp = schedulingData[name][TaskName]["time:timestamp"]
    if "StandByMessage" in schedulingData[name][TaskName].keys(): 
        timestampStandBy = schedulingData[name][TaskName]["StandByMessage"]["time:timestamp"]
...
def messageIntermediateThrowEvent(elements, element, script, schedulingResult, schedulingData):
    from simulator.simulationFunctionsScheduler.generateFunction import generateFunctionScheduler
    if any(element.id_bpmn in schedulingDataByInstance for schedulingDataByInstance in schedulingData.values()):
        functionStr = f"""
def {element.id_bpmn}(env, name):
...
def timerIntermediateCatchEvent(elements, element, script, schedulingResult, schedulingData):
    from simulator.simulationFunctionsScheduler.generateFunction import generateFunctionScheduler
    if any(element.id_bpmn in schedulingDataByInstance for schedulingDataByInstance in schedulingData.values()):
        functionStr = f"""
def {element.id_bpmn}(env, name):
    schedulingData = {schedulingData}
    duration = schedulingData[name][TaskName]["bpmn:time"]
    timestamp = schedulingData[name][TaskName]["time:timestamp"]
    simulationResults[name].append(
f'''
        <event>
...
def endEvent(elements, element, script, schedulingResult, schedulingData):
    from simulator.simulationFunctionsScheduler.generateFunction import generateFunctionScheduler
    functionStr = f"""
def {element.id_bpmn}(env, name):
    simulationResults[name].append(
