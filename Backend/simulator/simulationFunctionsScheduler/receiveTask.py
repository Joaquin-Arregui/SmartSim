def receiveTask(elements, element, script, schedulingResult, schedulingData):
...
def {element.id_bpmn}(env, name):
    def executeTask(env, TaskName, name, schedulingData, execution):
        ...
        if "StandByMessage" in schedulingData[name][TaskName].keys(): 
            timestampStandBy = schedulingData[name][TaskName][execution]["StandByMessage"]["time:timestamp"]
            endTime = schedulingData[name][TaskName][execution]["StandByMessage"]["time:endTime"]
