...
def exclusiveGateway(elements, element, script, schedulingResult, schedulingData):
...
def {element.id_bpmn}(env, name):
    ...
    subTask = schedulingData[name]['{element.id_bpmn}']["bpmn:subTask"]
    timestamp = schedulingData[name]['{element.id_bpmn}']["time:timestamp"]
...
f'''
        <event>
            ...
            <string key="bpmn:subTask" value="{{subTask}}"/>
...
f'''
    ...
    for elem in possibleElements:
        if ('def ' + elem + '(env, name)') not in script:
            extendedScript = generateFunctionScheduler(elements, elem, extendedScript, schedulingResult, schedulingData)
    return extendedScript

def parallelGateway(elements, element, script, schedulingResult, schedulingData):
    from simulator.simulationFunctionsScheduler.generateFunction import generateFunctionScheduler
...
def inclusiveGateway(elements, element, script, schedulingResult, schedulingData):
    from simulator.simulationFunctionsScheduler.generateFunction import generateFunctionScheduler
    possibleElements, percents = getPercentOfBranches(elements, element.id_bpmn)
    if any(element.id_bpmn in schedulingDataByInstance for schedulingDataByInstance in schedulingData.values()):
        functionStr = f"""
